...
describe('reencryptionWorker', () => {
    ...
    it('should emit re-encrypted chunk', async () => {
        ...
        const encryptedChunkWithOldKey = await crypto.subtle.encrypt({ name: options.ASYMMETRIC_ENCRYPTION_ALGORITHM }, keysOld.publicKey, bytes);

        return new Promise(resolve => {
            reencryptionWorker([encryptedChunkWithOldKey, '', '', '', options], () => {}, async (result) => {
                const decryptedChunk = await crypto.subtle.decrypt({ name: options.ASYMMETRIC_ENCRYPTION_ALGORITHM }, keysNew.privateKey, result.chunk);
